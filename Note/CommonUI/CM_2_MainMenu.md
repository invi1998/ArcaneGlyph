...
PS手柄适配UE小工具[DS4Windows Download | Ryochan7 DS4 Version (2025)](https://ds4-windows.com/download/ryochan7-ds4windows/)

在手柄识别完成后，我们如果直接开始游戏，发现手柄并不能操作菜单，这是因为我们需要在UI页面里设置聚焦的子控件才能让手柄正常工作

![image-20250607165426290](.\image-20250607165426290.png)

如图，我们希望手柄在进入菜单页面后，自动聚焦到开始游戏控件上。

![image-20250607165624807](.\image-20250607165624807.png)

然后我们就发现现在手柄可以正常导航使用了，但是此时手柄导航会导致导航到的控件出现蓝色边框，为了消除这个边框，我们需要在项目设置里将渲染聚焦规则改为从不。

![image-20250607165849805](.\image-20250607165849805.png)

同样，我们需要为手柄创建操作数据资产

![image-20250607172315663](.\image-20250607172315663.png)

注意这里，选择游戏手柄名称的时候，除非我们修改源码，否则UE只提供了一个默认的手柄名称供我们选择（通用Generic），然后我们回到CommonUI输入的项目设置这里，看到手柄名称默认为Windows，我们需要将其修改为资产里同名名称Generic。

![image-20250607172535493](.\image-20250607172535493.png)

然后去数据表格里新增手柄操作的输入映射

![image-20250607172741377](.\image-20250607172741377.png)

然后就是新增确认映射，因为对于鼠标操作来说，我们不需要确认，鼠标可以自由的点点点，而手柄才需要我们分配具体哪个按钮是确认，哪个是返回，所以对于确认的键盘鼠标这里，映射为空

![image-20250607172837480](.\image-20250607172837480.png)
