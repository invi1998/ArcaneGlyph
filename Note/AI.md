...
然后我们就可以完善此前的行为树，添加完整的攻击逻辑

![image-20250225154637103](.\image-20250225154637103.png)

注意这里，我们在攻击分支结束时，又执行了一次旋转任务，这个任务是为了在敌人退出近战攻击分支之前调整他们的旋转，AI的正确旋转很重要，因为它会极大地影响在侧移分支下选择的侧移位置，因为EQS查询点位的结果一个最重要的依据就是AI的朝向，所以AI有一个目标清晰的朝向很重要。



# Actor 是否具有标签装饰器

目前，我们行为树的问题是，当我们攻击AI的时候，AI如果正处于向外走侧移状态（打完收工状态），那么，在受到我们的攻击的时候，并不会停下，这是比较反直觉的行为，它应该进入防御状态，格挡状态之类的，在行为树上要对我们的攻击行为要有一定的反馈分支。

所以我将使用装饰器来检查我们的AI是否具有某个标签。如果答案是肯定的，那么我将中止所有右侧的分支并开始执行这个防御分支。

![image-20250225203147317](.\image-20250225203147317.png)

如上，这是GameplayTagCondition这个UE原生的关于是否包含GameTag的装饰器。我们仔细对比其他装饰器，可以发现，该装饰器是没有流程控制选项的，即没有观察者，那么导致的结果就是我们无法像其他装饰器那样中断其他分支流程，这使得这个建筑装饰器在我们的情况下几乎变得无用。

![image-20250225203332426](.\image-20250225203332426.png)

所以，为了实现带流程控制的GameplayTag装饰器，我们又得创建自己的装饰器节点。我们在蓝图中直接基于`BTDecorator_BlueprintBase`创建。

![image-20250225203826216](.\image-20250225203826216.png)

如图，重写performConditionCheckAI函数，然后判断传入Actor是否具有指定的gameplayTag，同时为了接口灵活，我们还添加了一个结果翻转选项，如果翻转，有就是没有，没有就是有。
...
# 解决Boss类低移速的巨型怪物AI卡死问题

![image-20250306130051841](.\image-20250306130051841.png)

对于一些巨型AI，我们希望他们的移速很低，比如100左右，当我们这样设置的时候，在某些情况下会出现因为导航避让出现卡住的现象，如果我们打开调试

`ai.crowd.DebugSelectedActors 1`

然后按`F8`，查看AI的避让调试框，可以看到，白框明显比我们的AI角色小得多，红色框是我们的碰撞查询范围，白色框是我们的避让查询访问，白色框（避让查询）的大小受我们角色的最大行走速度的影响

![image-20250306130135933](.\image-20250306130135933.png)

如果我们将角色最大行走速度改大点，是可以解决这个问题

![image-20250306130641291](.\image-20250306130641291.png)

![image-20250306130720707](.\image-20250306130720707.png)

可是这种解决方案并不一定是我们想要的，巨型Boss有些时候我们就是希望它能有一个较低的移速。

我们可以调整一个属性来减轻我们刚才看到的问题，我们需要回到我们的 AI 控制器，我们可以修改这个CollisionQueryRange，让它适配我们的角色的行走速度，比如，这里改为100

![image-20250306131007581](.\image-20250306131007581.png)
